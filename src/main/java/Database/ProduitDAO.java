...
public class ProduitDAO {//Produit Data Access Object par nomenclature
    ...
    private static final String MOT_DE_PASSE = "tuL0iiVnn^Ye/fe";
    ...
    private Produit mapResultSetToProduit(ResultSet resultSet) throws SQLException {
        ...
        switch (type) {
            case "Vetement":
                produit = new Vetement(id,nom, prix, stock, taille);
                break;
            case "Chaussure":
                produit = new Chaussure(id,nom, prix, stock, pointure);
                break;
            case "Accessoire":
                produit = new Accessoire(id,nom,prix,stock);
                break;
            default:
                produit = new Produit(id,nom, prix, stock);
                break;
        }
    ...
    public void mettreAJourProduit(Produit produit) {
        try (Connection connexion = DriverManager.getConnection(URL, UTILISATEUR, MOT_DE_PASSE)) {
            ...
            } else if (produit.getNbex() == 0){
                String deleteRequete = "DELETE FROM produits WHERE id=?";
                try (PreparedStatement deleteStatement = connexion.prepareStatement(deleteRequete)) {
                    deleteStatement.setInt(1, produit.getId());
